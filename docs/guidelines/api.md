...
- `user_id`、`owner_id`、`role`、`admin`、`tenant_id`、`price`、`status`、`created_by`、`updated_by` など、所有・権限・金額・状態遷移・監査に関わる値は、原則として frontend からの params をそのまま信用しない。
...
- 一覧 API や詳細 API で関連データを返す場合は、N+1 query が発生していないか確認する。
- 必要に応じて `includes` など、既存の eager loading 方針に従う。
- 1つの request で複数 record を更新し、途中失敗で不整合が起きる場合は transaction を使う。
...
- migration を追加・変更した場合は、repository の方針に従って `db/schema.rb` または `db/structure.sql` が更新されていることを確認する。
...
- 既存 API の response shape、status code、route を変更する場合は、breaking change にならないか確認する。
- breaking change が必要な場合は、専用 Issue で扱い、PR に影響範囲を明記する。
