...
| StatusSegmentedControl on card | update status | `PATCH /api/spots/:id` | frontend helper/action | Backend route exists; frontend mutation not yet implemented. |
...
- Guideline points for this Issue: DB 更新の最終判断は Rails API に置く。二重送信を防ぐ。失敗時に UI を戻すか再取得する方針を明示する。
